...
function gateKeeper(req, res, next) {
  const stringy = req.get('x-username-and-password');
  const {user, pass} = queryString.parse(stringy);
  const validatedUser = USERS.find(function (possibleUser) {
    return possibleUser.userName === user;
  })
